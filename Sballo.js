...
function ShowInput() {
    ...
    cognome = cognome.replace(/[^A-Za-z]/g, '');
    ...
    if (para) { para.classList.remove('show'); para.innerHTML = ''; }

    // validazioni minime dei campi
    if (!nome || !cognome) {
      if (errorBox) { errorBox.textContent = 'Inserisci nome e cognome.'; errorBox.classList.add('show'); }
      return;
    }
     //APPUNTI
    ...
    //STAMPA SU PARAGRAFO (già definito sopra)
   
     ///####################ANNO DATI##############################///
   
    var dataDiNascita = document.getElementById('dateofbirth').value;
    if (!dataDiNascita || !/^\d{4}-\d{2}-\d{2}$/.test(dataDiNascita)) {
      if (errorBox) { errorBox.textContent = 'Inserisci una data di nascita valida.'; errorBox.classList.add('show'); }
      return;
    }
    ...
    var annoCompleto = data.getFullYear();
    if (annoCompleto < 1920) {
      if (errorBox) { errorBox.textContent = 'Inserisci una data di nascita valida.'; errorBox.classList.add('show'); }
      return;
    }
     ...
     var giorno = data.getDate();
      ///#################### SESSO ##############################///
      // Supporta <select id="gender"> e fallback ai radio #maschio/#femmina
      var genderValue = null;
      var genderSelect = document.getElementById('gender');
      if (genderSelect && (genderSelect.value === 'M' || genderSelect.value === 'F')) {
        genderValue = genderSelect.value;
      } else {
        var femmina = document.getElementById("femmina");
        var maschio = document.getElementById("maschio");
        if (femmina && femmina.checked) genderValue = 'F';
        if (maschio && maschio.checked) genderValue = 'M';
      }
      if (genderValue !== 'M' && genderValue !== 'F') {
        if (errorBox) { errorBox.textContent = 'Seleziona il genere.'; errorBox.classList.add('show'); }
        return;
      }
   ...
   //#VERSIONE STATICA
    ...
    // normalizza input località
    if (!localita) {
      if (errorBox) { errorBox.textContent = 'Inserisci il luogo di nascita.'; errorBox.classList.add('show'); }
      return;
    }
    localita = localita.toUpperCase().trim();

    if (paesiEsteri && paesiEsteri[localita]) {
      // Paese estero trovato, usa direttamente il codice Zxxx
      localita_presa = paesiEsteri[localita];
    } else {
      for (var i = 0; i <= provincia.length; i++) {
        if (localita == provincia[i]) {
  ...
  if (!localita_presa) {
     if (errorBox) { errorBox.textContent = 'Comune/Paese non riconosciuto.'; errorBox.classList.add('show'); }
      return;
    }

   // Giorno sempre su due cifre (maschi 01..31, femmine 41..71)
   var giornoStr = String(giorno).padStart(2, '0');
   var CF = consonanti_prese_cognome + consonanti_prese_nome + anno + mese + giornoStr + localita_presa;
...
// Esporta la funzione nel global scope per compatibilità con onclick inline in WordPress
if (typeof window !== 'undefined') {
  window.ShowInput = ShowInput;
}

// Popola il datalist dei comuni se presente nella pagina WordPress
function attachCFUIHandlers() {
  var datalist = document.getElementById('comuniList');
  var dateInput = document.getElementById('dateofbirth');
  if (dateInput) {
    var todayStr = new Date().toISOString().split('T')[0];
    if (dateInput.max !== todayStr) {
      dateInput.setAttribute('max', todayStr);
    }
  }
  if (datalist) {
    try {
      // Comuni italiani (ordine originale)
      ...
      // Paesi esteri aggiunti dopo (senza ordinamento specifico)
      if (paesiEsteri && typeof paesiEsteri === 'object') {
        var fragment2 = document.createDocumentFragment();
        for (var nomePaese in paesiEsteri) {
          if (!Object.prototype.hasOwnProperty.call(paesiEsteri, nomePaese)) continue;
          var option2 = document.createElement('option');
          option2.value = nomePaese;
          fragment2.appendChild(option2);
        }
        datalist.appendChild(fragment2);
      }
    } catch (e) {
      // ignora se l'array non è disponibile
    }
  }

  var calcBtn = document.getElementById('but');
  if (calcBtn && typeof ShowInput === 'function' && !calcBtn._cfBound) {
    calcBtn.addEventListener('click', ShowInput);
    calcBtn._cfBound = true;
  }
}

// Esegui al DOM pronto e anche subito se il DOM è già carico (footer WP)
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', attachCFUIHandlers);
} else {
  attachCFUIHandlers();
}
